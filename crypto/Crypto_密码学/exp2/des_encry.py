...
def encry_byte(plaintext, key, path):
    ...
    with open(path + '.des', 'ab') as file_w:
        num = 0
        while num * 8 < len(ciphertext_bin):
            byte_bin = ''.join(ciphertext_bin[num * 8:(num + 1) * 8])
            byte_hex = hex(int(byte_bin, 2) + 256)[3:]
            byte_hex_bin = binascii.a2b_hex(byte_hex)
            file_w.write(byte_hex_bin)
            num += 1


def encry(path, key):
    with open(path, 'rb') as file_r:
        plaintext = file_r.read()
    # 判断是否为8字节整数倍
    n = len(plaintext) % 8
    if n:
        while n < 8:
    ...
    while count * 8 < length:
        plaintext_t = plaintext[count * 8:(count + 1) * 8]
        encry_byte(plaintext_t, key, path)
        count += 1
...
def main():
    ...
    # 清空文件
    with open(path + '.des', 'wb') as file_wb:
        pass
    encry(path, key)
    print 'The cipher file path: ' + path + '.des'
